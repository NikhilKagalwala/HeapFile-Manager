...
int main(int argc, char **argv) {
    cout << "Testing the relation interface" << endl
         << endl;
    ...
    if (status != OK)
        ...
        while (((status = scan1->scanNext(rec2Rid)) != FILEEOF)) {
            ...
            if (status != OK) break;
            // cout << rec1.s << " " << ((RECORD *)(dbrec2.data))->s;
            if (memcmp(&rec1, dbrec2.data, sizeof(RECORD)) != 0)
                cout << "err0r reading record " << i << " back" << endl;
            i++;
        }
        ...
        while ((status = scan1->scanNext(rec2Rid)) != FILEEOF) {
            ...
            if ((i % 2) != 0) {
                ...
                if ((status != OK) && (status != NORECORDS)) {
                    cout << "err0r status return from deleteRecord" << status << endl;
                    error.print(status);
                }
            }
            i++;
    ...
    cout << endl
         << "scan dummy.03 using the predicate < num/2 " << endl;
    j = num / 2;
    ...
    if ((status == INVALIDRECLEN) || (status == NOSPACE)) {
        cout << endl
             << "passed large record insert test" << endl;
    } else {
    ...
    if ((status = destroyHeapFile("dummy.04")) != OK) {
        cout << endl
             << "got err0r status return from destroy file" << endl;
        error.print(status);
    }
    delete bufMgr;
